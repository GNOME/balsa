...

  if (!LOCAL) {			/* open new connection if no recycle */
	...
	net_aopen (NIL,&mb,"/etc/rimapd",usr)) {
    ...
    if (LOCAL->netstream) {	/* if have a connection */
	  ...
	  !((LOCAL->use_auth &&
	     (LOCAL->use_authanon || !(mb.anoflag || stream->anonymous))) ?
	    imap_auth (stream,&mb,tmp,usr) : imap_login (stream,&mb,tmp,usr))){
...

  if (LOCAL->netstream) {	/* still have a connection? */
    ...
    if (LOCAL->user) sprintf (tmp + strlen (tmp),"/imap/user=%s}",LOCAL->user);
...
unsigned long imap_auth (MAILSTREAM *stream,NETMBX *mb,char *tmp,char *usr)
{
  ...
  if (mb->anoflag || stream->anonymous) {
    ...
    sprintf (tmp,"%s AUTHENTICATE ANONYMOUS",tag);
				/* get local host name */
    s = net_localhost (LOCAL->netstream);
    if (imap_soutr (stream,tmp) &&
	imap_challenge (stream,&trial) && !trial &&
  ...
  else for (ua = LOCAL->use_auth; LOCAL->netstream && ua;) {
    ...
    do {			/* gensym a new tag */
      ...
      if (imap_soutr (stream,tmp) &&
	  (*at->client) (imap_challenge,imap_response,mb,stream,&trial,usr)) {
				/* abort if don't have tagged response */
	while (strcmp ((reply = imap_reply (stream,tag))->tag,tag))
	  imap_soutr (stream,"*");
				/* done if got success response */
    ...
    while (LOCAL->netstream && !LOCAL->byeseen &&
  ...
  if (arglst = args) while (arg = *arglst++) {
	...
	*s++ = ')';		/* wrap parens around string */
    ...
    case ASTRING:		/* atom or string, must be literal? */
      st.size = strlen (st.data = (unsigned char *) arg->text);
...
long imap_soutr (MAILSTREAM *stream,char *string)
{
  ...
  return net_soutr (LOCAL->netstream,strcat (strcpy (tmp,string),"\015\012"));
}

/* IMAP get reply
 * Accepts: MAIL stream
...
void imap_parse_response (MAILSTREAM *stream,char *text,long errflg,long ntfy)
{
  ...
  if ((*text == '[') && (t = strchr (s = text + 1,']')) &&
      ((i = t - s) < IMAPTMPLEN)) {
    LOCAL->tmp[i] = '\0';	/* make uppercase copy of text code */
    ucase (strncpy (LOCAL->tmp,s,i));
    ntfy = NIL;			/* suppress mm_notify for normal SELECT data */
    if (!strncmp (LOCAL->tmp,"UIDVALIDITY ",12))
      stream->uid_validity = strtoul (LOCAL->tmp+12,NIL,10);
    else if (!strncmp (LOCAL->tmp,"UIDNEXT ",8))
      stream->uid_last = strtoul (LOCAL->tmp+12,NIL,10) - 1;
	...
	if (!strcmp (s,"\\SEEN")) stream->perm_seen = T;
      ...
      else if (!strncmp (LOCAL->tmp,"REFERRAL ",9))
  ...
  if (*t++ == '(') while (*t != ')') {
    ...
    if ((*t != '{') && (*t != '"') && (s = strpbrk (t," )"))) {
      ...
      *s = '\0';		/* tie off atom and copy it*/
      stc->text.size = strlen (stc->text.data = (unsigned char *) cpystr (t));

...

  ...
  default:
    ...
    if (!ret && ((r = env_parameters (function,value)) ||
		 (r = tcp_parameters (function,value)))) ret = r;
    break;
  }
  return ret;
}
	  ...
	  !(((charset[0] == 'U') || (charset[0] == 'u')) &&
	    ((((charset[1] == 'S') || (charset[1] == 's')) &&
	     ...
	     (((charset[1] == 'U') || (charset[1] == 'u')) &&
	      ((charset[2] == 'T') || (charset[2] == 't')) &&
	      ((charset[3] == 'F') || (charset[3] == 'f')) &&
	      (charset[4] == '-') && (charset[5] == '8') && !charset[6])))) {
			   ...
			   long flags)
{
  ...
  while (src < end) {		/* process header */
    ...
    *t = '\0';			/* tie off */
	...
	if ((hdrs->text.size == i) &&
	    !strncmp (tmp,ucase (strncpy (tst,hdrs->text.data,(size_t) i)),
		      (size_t) i)) notfound = NIL;
...
{
  THREADER *t;
  for (t = &mailthreadlist; t; t->next) if (!strcmp (type,t->name))
...
SEARCHHEADER *mail_newsearchheader (char *line,char *text)
{
  ...
  hdr->line.size = strlen (hdr->line.data = (unsigned char *) cpystr (line));
  hdr->text.size = strlen (hdr->text.data = (unsigned char *) cpystr (text));
  return hdr;
}
...
NETSTREAM *net_open (NETDRIVER *dv,char *host,char *service,unsigned long prt)
{
  ...
  if (s = strchr (host,':')) {	/* port number specified? */
    size_t hs = s++ - host;
    char *hst = strncpy ((char *) fs_get (hs + 1),host,hs);
    ...
    else tstream = (*dv->open) (hst,NIL,prt);
    fs_give ((void **) &hst);
  }

...

  if (nstream && mb.user[0]) {	/* user name specified? */
				/* yes, get password */
    mm_login (&mb,mb.user,tmp,0);
    if (!tmp[0]) {		/* user refused to give a password */
      mm_log ("Login aborted",ERROR);
      nntp_close (nstream);	/* punt stream */
      nstream = NIL;
    }
				/* do NNTP authentication */
    switch (nntp_send_work (nstream,"AUTHINFO USER",mb.user)) {
    case NNTPAUTHED:		/* all done */
      break;
    case NNTPWANTPASS:		/* send password */
      if (nntp_send_work (nstream,"AUTHINFO PASS",tmp) == NNTPAUTHED) break;
    default:			/* error */
  ...
  if (nstream) {		/* now try to open newsgroup */
    ...
    sprintf (tmp,"{%s:%lu/nntp%s%s}#news.%s",net_host (nstream->netstream),
	     net_port (nstream->netstream),LOCAL->user ? "/user=" : "",
	     LOCAL->user ? LOCAL->user : "",mbx);
...

	...
	stream->silent = T;	/* don't notify main program yet */
				/* set the cache to the guesstimate */
...
long nntp_overview (MAILSTREAM *stream,char *sequence,overview_t ofn)
{
  ...
  while (*sequence) {
    for (s = tmp; *sequence && (*sequence != ','); sequence++)
      *s++ = (*sequence == ':') ? '-' : *sequence;
    *s++ = '\0';
    if (nntp_send (LOCAL->nntpstream,"XOVER",tmp) != NNTPOVER) return NIL;
	  ...
	  *v++ = '\0';		/* tie off subject */
	  ...
	  while (*t) {		/* flush leading "re:" and whitespace */
		...
		((t[1] == 'E') || (t[1] == 'e')) && (t[2] == ':')) t += 3;
				...
				/* have a subject? */
	  if (src.size = strlen (src.data = (unsigned char *) t)) {
...
{
  long trial = 0;
  long ret = nntp_send_work (stream,command,args);
  while (((ret == NNTPWANTAUTH) || (ret == NNTPWANTAUTH2)) &&
    ...
    mm_login (&mb,usr,pwd,trial++);
				/* abort if he refuses to give password */
    if (!*pwd) trial = nntp_maxlogintrials;
				/* try it if user gave one */
    else switch (nntp_send_work (stream,"AUTHINFO USER",usr)) {
    case NNTPWANTPASS:
      if (nntp_send_work (stream,"AUTHINFO PASS",pwd) != NNTPAUTHED) break;
    case NNTPAUTHED:

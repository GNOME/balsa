...

  ...
  if (sock < 0) mm_log (tmp,ERROR);
    ...
    stream->localhost = cpystr ((he = gethostbyname (tmp)) ? he->h_name : tmp);
  }
		     ...
		     unsigned long port)
{
  ...
  tmo.tv_sec = tcptimeout_open;	/* initialize timeouts */
  tmo.tv_usec = 0;
...
TCPSTREAM *tcp_aopen (NETMBX *mb,char *service,char *usrbuf)
{
  ...
  else if (he = gethostbyname (lcase (strcpy (hostname,mb->host))))
    strcpy (hostname,he->h_name);
				/* make command pipes */
...

  ...
  stream->localhost = cpystr ((he = gethostbyname (hostname)) ?
			      he->h_name : hostname);
...
char *tcp_clienthost ()
{
  if (!myClientHost) {
    ...
    myClientHost =
      cpystr (getpeername (0,(struct sockaddr *) &from,&fromlen) ? "UNKNOWN" :

...
static void libbalsa_address_book_class_init(LibBalsaAddressBookClass *klass);
static void libbalsa_address_book_init(LibBalsaAddressBook *ab);
static void libbalsa_address_book_destroy (GtkObject *object);

static void libbalsa_address_book_real_save_config(LibBalsaAddressBook *ab, const gchar *prefix);
static void libbalsa_address_book_real_load_config(LibBalsaAddressBook *ab, const gchar *prefix);
...
GtkType
libbalsa_address_book_get_type (void)
{
	...
	if (!address_book_type) {
		...
		address_book_type = gtk_type_unique(gtk_object_get_type(), &address_book_info);
	}
...
libbalsa_address_book_class_init(LibBalsaAddressBookClass *klass)
{
	...
	libbalsa_address_book_signals[STORE_ADDRESS] =
		gtk_signal_new("store-address",
			       GTK_RUN_FIRST,
			       object_class->type,
			       GTK_SIGNAL_OFFSET(LibBalsaAddressBookClass, store_address),
	...
	libbalsa_address_book_signals[SAVE_CONFIG] =
		gtk_signal_new("save-config",
			       GTK_RUN_FIRST,
			       object_class->type,
			       GTK_SIGNAL_OFFSET(LibBalsaAddressBookClass, save_config),
	...
	libbalsa_address_book_signals[LOAD_CONFIG] =
		gtk_signal_new("load-config",
			       GTK_RUN_FIRST,
			       object_class->type,
			       GTK_SIGNAL_OFFSET(LibBalsaAddressBookClass, load_config),
			       gtk_marshal_NONE__POINTER,
			       GTK_TYPE_NONE, 1, GTK_TYPE_POINTER);

	gtk_object_class_add_signals(object_class, libbalsa_address_book_signals, LAST_SIGNAL);
...
libbalsa_address_book_destroy (GtkObject *object)
{
	...
	g_free(ab->config_prefix); ab->config_prefix = NULL;

	g_free(ab->name); ab->name = NULL;
...
libbalsa_address_book_new_from_config(const gchar *prefix)
{
	...
	if ( got_default == TRUE ) {
		libbalsa_information(LIBBALSA_INFORMATION_WARNING, _("Cannot load address book %s"), prefix);
		return NULL;
	}
	
	type = gtk_type_from_name(type_str);
	if ( type == 0 ) {
		libbalsa_information(LIBBALSA_INFORMATION_WARNING, _("No such address book type: %s"), type_str);
		g_free(type_str);
		return NULL;
	}
	...
	if ( address_book == NULL ) {
		libbalsa_information(LIBBALSA_INFORMATION_WARNING, _("Could not create a address book of type %s"), type_str);
		g_free(type_str);
		return NULL;
	}
...
void
libbalsa_address_book_store_address(LibBalsaAddressBook *ab, LibBalsaAddress *address)
{
	g_return_if_fail ( LIBBALSA_IS_ADDRESS_BOOK(ab) );
	g_return_if_fail ( LIBBALSA_IS_ADDRESS(address) );

	gtk_signal_emit(GTK_OBJECT(ab), libbalsa_address_book_signals[STORE_ADDRESS], address);
}


void
libbalsa_address_book_save_config(LibBalsaAddressBook *ab, const gchar *prefix)
{
	g_return_if_fail ( LIBBALSA_IS_ADDRESS_BOOK(ab) );

	gnome_config_push_prefix(prefix);
	gtk_signal_emit(GTK_OBJECT(ab), libbalsa_address_book_signals[SAVE_CONFIG], prefix);
	gnome_config_pop_prefix();
}

void
libbalsa_address_book_load_config(LibBalsaAddressBook *ab, const gchar *prefix)
{
	g_return_if_fail ( LIBBALSA_IS_ADDRESS_BOOK(ab) );

	gnome_config_push_prefix(prefix);
	gtk_signal_emit(GTK_OBJECT(ab), libbalsa_address_book_signals[LOAD_CONFIG], prefix);
	gnome_config_pop_prefix();
}

static void
libbalsa_address_book_real_save_config(LibBalsaAddressBook *ab, const gchar *prefix)
{
	g_return_if_fail ( LIBBALSA_IS_ADDRESS_BOOK(ab) );
...
static void
libbalsa_address_book_real_load_config(LibBalsaAddressBook *ab, const gchar *prefix)
{
	g_return_if_fail ( LIBBALSA_IS_ADDRESS_BOOK(ab) );

                     ...
                     GtkIconSize         size)
{
        ...
        if (G_IS_THEMED_ICON(icon)) {
            ...
            if (icon_names != NULL) {
                for (i = 0; pixbuf == NULL && icon_names[i] != NULL; i++) {
                    pixbuf = gtk_icon_theme_load_icon(icon_theme, icon_names[i], width, GTK_ICON_LOOKUP_FORCE_SIZE, NULL);
                }
    ...
    if (pixbuf == NULL) {
        /* load the default pixbuf, i.e. for application/octet-stream */
        pixbuf = gtk_icon_theme_load_icon(icon_theme, "application-octet-stream", width, GTK_ICON_LOOKUP_FORCE_SIZE, NULL);
    }

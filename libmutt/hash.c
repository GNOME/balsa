...
int hash_string (unsigned char *s, int n)
{
  int h = 0;
  while (*s)
    h += *s++;
...
int hash_insert (HASH *table, char *key, void *data, int allow_dup)
{
...
void *hash_find_hash (HASH *table, int hash, char *key)
{
...
void hash_delete_hash (HASH *table, int hash, char *key, void *data,
		       void (*destroy) (void *))
{
  struct hash_elem *ptr = table->table[hash];
...
void hash_destroy (HASH **ptr, void (*destroy) (void *))
{
  ...
  for (i = 0 ; i < pptr->nelem; i++)
  {
    for (elem = pptr->table[i]; elem; )
    {
      ...
      if (destroy)
	destroy (tmp->data);
      memset (tmp, 0, sizeof (struct hash_elem));
      free (tmp);
    }
  }
  free (pptr);

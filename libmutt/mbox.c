...
int mbox_sync_mailbox (CONTEXT *ctx)
{
   ...
   */
  for (i = 0 ; i < ctx->msgcount && !ctx->hdrs[i]->deleted && !ctx->hdrs[i]->changed ; i++)
    ;
  if (i == ctx->msgcount)
  { 
  ...
  for (i = first, j = 0; i < ctx->msgcount; i++)
  {
	...
	if (fputs (MMDF_SEP, fp) == EOF)
      ...
      if (mutt_copy_message (fp, ctx, ctx->hdrs[i], 0, CH_FROM | CH_UPDATE | CH_UPDATE_LEN) == -1)
	goto bail;
  ...
  if (fclose (ctx->fp) != 0 || i == -1)
  {
    ...
    snprintf (savefile, sizeof (savefile), "%s/mutt.%s-%s-%d",
	      Tempdir, Username, Hostname, getpid ());

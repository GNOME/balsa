...
balsa_address_book_config_new (LibBalsaAddressBook *address_book)
{
	...
	gnome_dialog_set_parent ( GNOME_DIALOG(abc->window), GTK_WINDOW(balsa_app.main_window));
	...
	if ( address_book == NULL ) {		
		...
		gtk_signal_connect(GTK_OBJECT(button), "clicked",
				   GTK_SIGNAL_FUNC(next_button_cb), (gpointer)abc);
		...
		gtk_container_add (GTK_CONTAINER (bbox), button);
		gtk_signal_connect (GTK_OBJECT (button), "clicked",
			(GtkSignalFunc) update_button_cb, (gpointer)abc);
		gtk_widget_show(button);
		abc->continue_button = button;
	...
	if ( address_book == NULL ) {
		if ( abc->create_type == LIBBALSA_TYPE_ADDRESS_BOOK_VCARD ) {
			gchar *path = gnome_file_entry_get_full_path ( GNOME_FILE_ENTRY(abc->ab_specific.vcard.path), FALSE );
			if ( path != NULL )
				address_book = libbalsa_address_book_vcard_new(name, path);
#ifdef ENABLE_LDAP
		} else if ( abc->create_type == LIBBALSA_TYPE_ADDRESS_BOOK_LDAP ) {
			gchar *host_name = gtk_entry_get_text ( GTK_ENTRY(abc->ab_specific.ldap.host_name) );
			gchar *base_dn = gtk_entry_get_text ( GTK_ENTRY(abc->ab_specific.ldap.base_dn) );
			address_book = libbalsa_address_book_ldap_new(name, host_name, base_dn);
#endif
		} else
			g_assert_not_reached();
		...
		address_book->name = g_strdup(gtk_entry_get_text(GTK_ENTRY(abc->name_entry)));
...
create_choice_page(AddressBookConfig *abc)
{
	...
	/* ... vCard Address Book */
	radio_button = gtk_radio_button_new_with_label (NULL, _("VCard Address Book (GnomeCard)"));
	gtk_box_pack_start (GTK_BOX (vbox), radio_button, FALSE, FALSE, 0);
	gtk_signal_connect (GTK_OBJECT (radio_button), "clicked", 
			    GTK_SIGNAL_FUNC(set_the_page), (gpointer) abc);
	gtk_object_set_user_data(GTK_OBJECT(radio_button), GINT_TO_POINTER(LIBBALSA_TYPE_ADDRESS_BOOK_VCARD));
	...
	gtk_signal_connect (GTK_OBJECT (radio_button), "clicked", 
			    GTK_SIGNAL_FUNC (set_the_page), (gpointer) abc);
	gtk_object_set_user_data(GTK_OBJECT(radio_button), GINT_TO_POINTER(LIBBALSA_TYPE_ADDRESS_BOOK_LDAP));
	gtk_widget_show (radio_button);
#else
...
create_vcard_page(AddressBookConfig *abc)
{
  ...
  if ( abc->address_book ) {
	  ...
	  entry = GTK_WIDGET(gnome_file_entry_gtk_entry(GNOME_FILE_ENTRY(abc->ab_specific.vcard.path)));

	  gtk_entry_set_text(GTK_ENTRY(abc->name_entry), abc->address_book->name);
...
create_ldap_page(AddressBookConfig *abc)
{
  ...
  gtk_table_attach (GTK_TABLE (table), abc->ab_specific.ldap.host_name, 1, 2, 1, 2,
		    GTK_EXPAND | GTK_FILL, GTK_FILL,
		    0, 10);
  ...
  gtk_table_attach (GTK_TABLE (table), abc->ab_specific.ldap.base_dn, 1, 2, 2, 3,
		    GTK_EXPAND | GTK_FILL, GTK_FILL,
		    0, 10);
  gtk_widget_show(abc->ab_specific.ldap.base_dn);

  abc->expand_aliases_button = gtk_check_button_new_with_label ( _("Expand aliases as you type"));
  gtk_table_attach(GTK_TABLE (table), abc->expand_aliases_button, 1, 2, 3, 4,
		   GTK_EXPAND|GTK_FILL, GTK_FILL,
		   0, 10);
  ...
  if ( abc->address_book ) {
	  ...
	  gtk_entry_set_text(GTK_ENTRY(abc->name_entry), abc->address_book->name);
	  gtk_entry_set_text(GTK_ENTRY(abc->ab_specific.ldap.host_name), ldap->host);
	  gtk_entry_set_text(GTK_ENTRY(abc->ab_specific.ldap.base_dn), ldap->base_dn);
	  gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(abc->expand_aliases_button), abc->address_book->expand_aliases);
  }
...
set_the_page(GtkWidget *button, AddressBookConfig *abc)
{
	...
	if ( gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(button)) ) {
		GtkType type = GPOINTER_TO_INT(gtk_object_get_user_data(GTK_OBJECT(button)));

...
static gboolean check_cyclic_tree(GNode* root, GNode* child);
...
static
GNode*
check_references(LibBalsaMessage* message,
		 GHashTable *id_table){
  ...
  */
  ...
  while(reference){
    ...
    if(check_cyclic_tree(foo, parent)){
      /* hmm.... */
...
adopt_child(GNode* parent, GNode* container){
  ...
  */
  ...
  if(check_cyclic_tree(container, parent)){
    /*printf("cyclic 2#\n");*/
    return;
  }
...
static
gboolean
check_cyclic_tree(GNode* root, GNode* child){
  gboolean find=FALSE;
  GNode* foo=NULL;
#if 0
...
static
GNode*
get_container(LibBalsaMessage* message,
	      GHashTable* id_table){
  GNode* container=NULL;
  ...
  */
  ...
  if(node->children!=NULL && node->data==NULL && 
	...
	root_set=g_slist_next(root_set);
      ...
      /*
        printf("prune: %x, parent(%x), next(%x), prev(%x), children(%x)\n",
               (int)node, (int)(node->parent),
               (int)(node->next), (int)(node->prev), (int)(node->children));
      */
      if(node->prev==NULL){

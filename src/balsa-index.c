...
static void transfer_messages_cb(GtkTreeSelection * selection,
                                 gpointer data);
...
balsa_index_load_mailbox_node (BalsaIndex * bindex, BalsaMailboxNode* mbnode)
{
    ...
    /* FIXME: this is an ugly way of doing it:
       ...
       default setting
    */
    balsa_index_set_threading_type(bindex, balsa_app.threading_type);
...
show_bmbl(GtkMenuItem * menuitem, BalsaIndex * bindex)
{
    ...
    GtkTreeSelection *selection =
        gtk_tree_view_get_selection(GTK_TREE_VIEW(bmbl));
    GtkRequisition req;
    ...
    g_signal_connect(G_OBJECT(selection), "changed",
                     G_CALLBACK(transfer_messages_cb),
                     (gpointer) bindex);
...
static void
transfer_messages_cb(GtkTreeSelection * selection, gpointer data)
{
    GtkTreeModel *model;
    GtkTreeIter iter;
...
balsa_index_set_threading_type(BalsaIndex * bindex, int thtype)
{
    ...
    bindex->mailbox_node->threading_type = thtype;

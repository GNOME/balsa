...
static void
balsa_information_list_button_cb ( GnomeDialog *dialog, gint button, gpointer data )
{
	switch ( button ) {
	case 0:
...
balsa_information_list (LibBalsaInformationType type, const char *fmt, ...)
{
	...
	if ( information_list == NULL ) {
		...
		information_dialog = gnome_dialog_new ( _("Balsa Information"), "Clear", GNOME_STOCK_BUTTON_CLOSE, NULL);
		/* Default is to close */
		gnome_dialog_set_default ( GNOME_DIALOG(information_dialog), 1 );
		gnome_dialog_set_parent ( GNOME_DIALOG ( information_dialog ), GTK_WINDOW( balsa_app.main_window ) );

		/* Reset the policy gnome_dialog_new makes itself non-resizable */
		gtk_window_set_policy ( GTK_WINDOW ( information_dialog ), TRUE, TRUE, FALSE );
		gtk_window_set_default_size ( GTK_WINDOW ( information_dialog ), 350, 200 );
		gtk_window_set_wmclass ( GTK_WINDOW(information_dialog), "Information", "Balsa" );

		gtk_signal_connect ( GTK_OBJECT(information_dialog), "clicked",
				     GTK_SIGNAL_FUNC(balsa_information_list_button_cb), NULL );

		/* A scrolled window for the list. */
		scrolled_window = gtk_scrolled_window_new ( NULL, NULL );
		gtk_scrolled_window_set_policy ( GTK_SCROLLED_WINDOW ( scrolled_window ), GTK_POLICY_AUTOMATIC, GTK_POLICY_AUTOMATIC );
		gtk_box_pack_start ( GTK_BOX( GNOME_DIALOG(information_dialog)->vbox ), scrolled_window, TRUE, TRUE, 1 );
		...
		gtk_container_add ( GTK_CONTAINER(scrolled_window), information_list );
		gtk_clist_set_column_auto_resize ( GTK_CLIST(information_list), 0, TRUE );

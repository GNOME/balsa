			...
			char delim)
{
    ...
    parent = balsa_find_dir(parent_name);
    if (parent == NULL) {
        parent = root;
	g_object_ref(parent);
    }
    ...
    url = libbalsa_imap_url(root->server, isi->fn);
    mbnode = remove_special_mailbox_by_url(url, &isi->special);
    g_free(url);
    if (mbnode != NULL)
        g_object_unref(mbnode);

...
static void mailbox_select_cb(BalsaMBList *, LibBalsaMailbox *,
...
static gboolean mblist_key_press_cb(GtkWidget * widget,
 ...
 /*PKGW*/ static void size_allocate_cb(GtkWidget * widget,
				       GtkAllocation * alloc);

static GtkWidget *mblist_create_context_menu(GtkCTree * ctree,
					     LibBalsaMailbox * mailbox);

static void mblist_drag_data_received(GtkWidget * widget,
				      GdkDragContext * context, gint x,
				      gint y,
				      GtkSelectionData * selection_data,
				      guint info, guint32 time);
static gboolean mblist_drag_motion(GtkWidget * widget,
				   GdkDragContext * context, gint x,
				   gint y, guint time);
static gboolean mblist_drag_leave(GtkWidget * widget,
				  GdkDragContext * context, guint time);


GtkWidget *
balsa_mailbox_list_window_new(BalsaWindow * window)
{
    GtkWidget *widget;
    ...
    gtk_object_set(GTK_OBJECT(mblw->ctree), "show_content_info",
		   balsa_app.mblist_show_mb_content_info, NULL);

    height =
	GTK_CLIST(mblw->ctree)->rows * GTK_CLIST(mblw->ctree)->row_height;
...
mblist_open_mailbox(LibBalsaMailbox * mailbox)
{
    ...
    if (c != -1) {
	page =
	    gtk_notebook_get_nth_page(GTK_NOTEBOOK(balsa_app.notebook), c);
		       ...
		       gpointer user_data)
{
	...
	if (event && event->button == 3) {
	    ...
	    gtk_menu_popup(GTK_MENU
			   (mblist_create_context_menu
			    (GTK_CTREE(bmbl), mailbox)), NULL, NULL, NULL,
	...
	if (event->type == GDK_BUTTON_PRESS && event->button == 3) {
	    ...
	    gtk_menu_popup(GTK_MENU
			   (mblist_create_context_menu
			    (GTK_CTREE(bmbl), NULL)), NULL, NULL, NULL,
			   NULL, event->button, event->time);
	    return TRUE;
	}
...
static void
mailbox_select_cb(BalsaMBList * bmbl, LibBalsaMailbox * mailbox,
			  ...
			  guint info, guint32 time)
{
  ...
  /*--*/
	...
	nb_received_messages =
	    (guint) (selection_data->length) / sizeof(LibBalsaMessage *);
	received_message_list =
	    (LibBalsaMessage **) (selection_data->data);

	for (received_message_count = 0;
	     received_message_count < nb_received_messages;
	     received_message_count++) {
	    current_message =
		received_message_list[received_message_count];
...
static gboolean
mblist_drag_leave(GtkWidget * widget, GdkDragContext * context, guint time)
{
    GtkCList *clist = GTK_CLIST(widget);

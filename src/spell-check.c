...
/* change_cb ()
 * 
 * Change the current word being checked to the selected suggestion.
 * */
static void 
...
/* learn_cb ()
 * 
 * Add the current word to the permanent dictionary.
 * */
static void 
...
balsa_spell_check_start (BalsaSpellCheck* spell_check)
{
        ...
        if (pspell_error_number (spell_error) != 0) {
                ...
                g_print (pspell_error_message (spell_error));
 ...
 * 
 * Learn the current word, either to the personal or session dictionaries 
 * */
static void 
balsa_spell_check_learn (BalsaSpellCheck* spell_check, LearnType learn_type)
{
        ...
        if (balsa_app.debug)
                g_print ("BalsaSpellCheck: Learn %s\n", word);
 ...
 * 
 * Replace the current word with the currently selected word.
 * */
static void 
balsa_spell_check_fix (BalsaSpellCheck* spell_check, gboolean fix_all)
{
        ...
        if (balsa_app.debug)
                g_print ("BalsaSpellCheck: Replace %s with %s\n", 
                         old_word, new_word);
...
balsa_spell_check_finish (BalsaSpellCheck* spell_check)
{
        ...
        if (balsa_app.debug)
                g_print ("BalsaSpellCheck: Finished\n");
...
/* setup_suggestions ()
 * 
 * A convenience function that retrieves the suggestions and places
...
setup_suggestions (BalsaSpellCheck* spell_check)
{
        ...
        while ((new_word = pspell_string_emulation_next (spell_check->suggestions)) != NULL) {
                if (balsa_app.debug)
                        g_print ("BalsaSpellCheck: Suggest %s\n", new_word);
...
check_word (BalsaSpellCheck* spell_check)
{
        ...
        if (word) {
                ...
                if (balsa_app.debug)
                        g_print ("BalsaSpellCheck: Check %s", word);
...
check_pspell_errors (PspellManager * manager)
{
  if (pspell_manager_error_number(manager) != 0) {
    g_print ("BalsaSpellCheck: Pspell Error: %s\n", pspell_manager_error_message(manager));
    return TRUE;
  }
...
next_word (BalsaSpellCheck* spell_check)
{
        ...
        if (regcomp (&quoted_rex, balsa_app.quote_regex, REG_EXTENDED)) {
                g_print ("BalsaSpellCheck: Quoted text regular expression compilation failed\n");
                return FALSE;
        }
        
        if (regcomp (&new_word_rex, new_word_regex, REG_EXTENDED | REG_NEWLINE)) {
                g_print ("BalsaSpellCheck: New word regular expression compilation failed\n");
                regfree (&quoted_rex);
                return FALSE;
        }
        ...
        while (!in_line && line && is_a_quote (line, &quoted_rex)) {
                if (balsa_app.debug) {
                        g_print ("BalsaSpellCheck: Ignore Quoted: %s\n", 
                                 line);
                }

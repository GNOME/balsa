...
balsa_store_address (GtkWidget * widget, gpointer index)
{
	...
	gtk_table_attach(GTK_TABLE(table), ab_option, 0, 2, 0, 1, GTK_FILL|GTK_EXPAND, GTK_FILL, 4, 4);

	for(cnt = 0; cnt < NUM_FIELDS; cnt++) {
		label = gtk_label_new( _(labels[cnt]) );
		entries[cnt] = gtk_entry_new();
      
		gtk_table_attach(GTK_TABLE(table), label, 0, 1, cnt+1, cnt + 2, GTK_FILL, 
				 GTK_FILL, 4, 4);
      
		gtk_label_set_justify(GTK_LABEL(label), GTK_JUSTIFY_LEFT);
      
		gtk_table_attach(GTK_TABLE(table), entries[cnt], 1, 2, cnt+1, cnt + 2,
...
store_address_dialog_button_clicked_cb(GtkWidget *widget, gint which, GtkWidget **entries)
{
	if(which == 0) {
		...
		address->full_name = g_strstrip(gtk_editable_get_chars(
			GTK_EDITABLE(entries[FULL_NAME]), 0, -1));
		address->first_name = g_strstrip(gtk_editable_get_chars(
			GTK_EDITABLE(entries[FIRST_NAME]), 0, -1));
		address->last_name = g_strstrip(gtk_editable_get_chars(
			GTK_EDITABLE(entries[LAST_NAME]), 0, -1));
		address->organization = g_strstrip(gtk_editable_get_chars(
